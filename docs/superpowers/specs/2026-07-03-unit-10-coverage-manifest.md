...
| 8 | 1:1 deletion (2026-07-09) | **Profile danger zone gains "delete conversation"** — the profile page is the 1:1's settings surface (members.tsx redirects 2-person conversations there), so both destructive actions live in its dangerZone slot. Delete = `leaveConversation` (writes "left" SystemEvent, self-revokes, forgets locally); counterpart keeps their copy; re-messaging starts a fresh thread (`find1to1Conversation` no longer matches). "Remove contact" now confirms via `RemoveContactDialog` with an opt-in "also delete our conversation" checkbox (default off). E2E: `conversation-delete-1to1.spec.ts`. |
