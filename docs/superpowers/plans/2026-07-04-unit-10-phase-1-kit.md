...
Port notes: Body = `flex-1 min-h-0 overflow-y-auto overflow-x-hidden bg-bg` + inline padding. PHeader: cluster verbatim (`min-h-[52px] shrink-0 flex items-center gap-[11px] px-3 border-b border-hairline bg-bg`); title `font-mono font-bold text-ui-title tracking-title truncate` (headMono true); sub row `mt-0.5 flex items-center gap-[5px]`; back button = tap + Icon back 20 text2; onTitle wraps avatar+title in one tap button (`flex-1 min-w-0 gap-[11px] text-left`). PTabBar: cluster verbatim (`h-[54px] shrink-0 flex items-stretch border-t border-hairline bg-bg`); each tab = tap + `flex-1 flex-col justify-center gap-[3px] py-[7px]`; icon 20 (`chat`/`people`) + label `font-mono text-ui-tab tracking-tab`, active `text-arcan-accent font-semibold` / inactive `text-dim font-medium`.
