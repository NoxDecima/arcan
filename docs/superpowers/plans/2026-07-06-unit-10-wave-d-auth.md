...
- **USER DECISION 2026-07-06 (walkthrough) — `tall` AuthSurface: center-when-fits, scroll-when-not.** The remaining `tall` surface (`BackupDisplayScreen`, `PScRecovery` parity cell) was top-pinned (`items-start`). Fixed in `src/ui/kit/auth-surface.tsx`: container drops `items-start`, content column gets `my-auto` (flex auto-margin centering). Auto margins absorb free space when content fits the viewport → vertical center; collapse to 0 on overflow → top-aligned scroll (unlike `justify-center` which clips the top). Proto parity copy (`AuthSurface` in `tests/parity/proto-cells.jsx`) patched to match (`alignItems: undefined` + `marginTop/marginBottom: 'auto'` on content column). Commit: `fix(auth): tall auth surfaces center when content fits, scroll only on overflow (walkthrough)`.
